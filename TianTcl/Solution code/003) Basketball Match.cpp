...
void real_process() {
	int path, bps; cin >> path >> bps;
	// int best = LM, diff, prev, pres;
	long unsigned best = LM, diff, dist[101] = {};
	loopi(path) {
		diff = 0; /* prev = 0;
		loopj(bps) {
			cin >> pres;
			diff = max(diff, pres-prev);
			prev = pres;
		} */
		loopj(bps) cin >> dist[j + 1];
		loopj(bps) diff = max(diff, dist[j + 1] - dist[j]);
		best = min(best, diff);
	} cout << best;
	// getchar();
	return;
