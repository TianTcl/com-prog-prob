...
void real_process() {
	ll digging[TT][2] = {0}, distance; cin >> distance;
	distance -= 2;
	digging[1][1] = digging[1][0] = 1;
	loopi1(distance - 1) {
		digging[i + 1][1] = digging[i][0] + digging[i][1];
		digging[i + 1][0] = digging[i][1];
	} cout << digging[distance][0] + digging[distance][1];
	// cout << endl; loop(2) getchar();
	return;
}
