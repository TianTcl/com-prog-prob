...
void real_process() {
	int radius, line; cin >> radius;
	loopi(2 * radius + 1) {
		loopj(2 * radius + 1) {
			line = (int)round(sqrt(pow(radius - j, 2) + pow(radius - i, 2)));
			if ((line % 3 == radius % 3 && line <= radius) || (radius > 1 && i == radius && j == radius && radius % 3 != 1)) cout << "# ";
			else cout << (line <= radius ? ". " : "  ");
		} cout << endl;
	} // getchar();
	return;
}
